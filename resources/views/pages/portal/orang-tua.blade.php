            ...
            @endif
                ...
                <div x-show="activeTab === 'home'" class="space-y-6">
                    ...
                    <div class="p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex justify-between items-center">
                        <div><span class="text-[9px] text-zinc-400 font-bold uppercase tracking-wider block">Wali Kelas</span>@if($homeroomTeacher)<span class="text-xs font-bold text-zinc-800 dark:text-zinc-200">{{ $homeroomTeacher->name }}</span><span class="text-[10px] text-zinc-400 dark:text-zinc-500 block mt-0.5">NIP: {{ $homeroomTeacher->nip ?? '-' }}</span>@else<span class="text-xs text-zinc-500">Belum terhubung</span>@endif</div>
