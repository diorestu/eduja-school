...
<div class="min-h-screen flex flex-col justify-center items-center p-6 bg-gray-50 dark:bg-[#08080c] transition-colors duration-500">
    ...
    <div class="w-full max-w-md bg-white dark:bg-[#12121a] border border-gray-200 dark:border-white/5 rounded-2xl shadow-xl p-8 transition-colors duration-500">
        <div class="text-center mb-8">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Daftarkan akun staf baru sekolah Anda</p>
        </div>
        ...
        <form action="{{ route('signup.store') }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="fname" class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Nama Depan</label>
                    <input type="text" id="fname" name="fname" value="{{ old('fname') }}" required placeholder="Nama depan"
                ...
                <div>
                    <label for="lname" class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Nama Belakang</label>
                    <input type="text" id="lname" name="lname" value="{{ old('lname') }}" required placeholder="Nama belakang"
                        class="w-full rounded-xl border border-gray-200 dark:border-white/10 bg-white/50 dark:bg-white/2 py-2.5 px-4 text-sm text-gray-800 dark:text-white focus:outline-none focus:border-brand-500 transition-all" />
                </div>
            </div>
            ...
            <div>
                <label for="password" class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Kata Sandi</label>
                <div x-data="{ showPassword: false }" class="relative">
                    <input :type="showPassword ? 'text' : 'password'" id="password" name="password" required placeholder="Minimal 6 karakter"
        ...
        <div class="mt-8 pt-6 border-t border-gray-100 dark:border-white/5 text-center text-xs text-gray-500">
            Sudah memiliki akun? <a href="/signin" class="text-brand-500 font-semibold hover:text-brand-600">Masuk Sekarang</a>
        </div>
    </div>
