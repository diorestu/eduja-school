...
<div class="auth-page grid min-h-screen md:grid-cols-[minmax(300px,0.78fr)_minmax(420px,1.22fr)]">
    ...
    <main class="auth-content flex items-center justify-center px-6 py-10 sm:px-10 md:px-16"><div class="w-full max-w-[430px]">
        ...
        <form action="{{ route('signin') }}" method="POST" class="space-y-5">@csrf
            <div><label for="email" class="mb-2 block text-sm font-bold">Alamat email</label><input class="auth-input w-full rounded-2xl px-4 py-3.5 text-sm" type="email" id="email" name="email" value="{{ old('email') }}" required placeholder="nama@sekolah.sch.id" autocomplete="email" @if($errors->has('email')) aria-invalid="true" aria-describedby="email-error" @endif>@error('email')<p id="email-error" class="mt-2 text-xs font-medium text-red-700" role="alert">{{ $message }}</p>@enderror</div>
            <div><div class="mb-2 flex items-center justify-between gap-4"><label for="password" class="block text-sm font-bold">Kata sandi</label><a href="/contact" class="auth-link whitespace-nowrap text-xs font-bold text-[var(--auth-teal)]">Butuh bantuan masuk?</a></div><div x-data="{ showPassword: false }" class="relative"><input class="auth-input w-full rounded-2xl px-4 py-3.5 pr-12 text-sm" :type="showPassword ? 'text' : 'password'" id="password" name="password" required placeholder="Masukkan kata sandi" autocomplete="current-password" @if($errors->has('password')) aria-invalid="true" aria-describedby="password-error" @endif><button type="button" @click="showPassword = !showPassword" class="auth-link absolute right-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-xl text-slate-400 hover:text-[var(--auth-teal)]" :aria-label="showPassword ? 'Sembunyikan kata sandi' : 'Tampilkan kata sandi'"><i x-show="!showPassword" class="bx bx-show text-lg"></i><i x-show="showPassword" class="bx bx-hide text-lg" style="display:none"></i></button></div>@error('password')<p id="password-error" class="mt-2 text-xs font-medium text-red-700" role="alert">{{ $message }}</p>@enderror</div>
            ...
            <button type="submit" class="auth-button w-full rounded-2xl py-4 text-sm font-bold text-white" data-submit-button><span data-submit-label>Masuk ke dasbor</span> <i class="bx bx-right-arrow-alt ml-1 align-middle text-lg" data-submit-icon></i></button>
        </form>
...
<script>document.addEventListener('DOMContentLoaded',()=>{document.querySelector('form[action="{{ route('signin') }}"]')?.addEventListener('submit',e=>{const b=e.currentTarget.querySelector('[data-submit-button]');if(!b)return;b.disabled=true;b.setAttribute('aria-busy','true');b.querySelector('[data-submit-label]').textContent='Memeriksa akun…';b.querySelector('[data-submit-icon]')?.remove();});});</script>
@endsection
