...
<div class="auth-page grid min-h-screen md:grid-cols-[minmax(300px,0.78fr)_minmax(420px,1.22fr)]">
    ...
    <main class="auth-content flex items-center justify-center px-6 py-10 sm:px-10 md:px-16"><div class="w-full max-w-[430px]">
        ...
        <form action="{{ route('signin') }}" method="POST" class="space-y-5">@csrf
            <div><label for="login" class="mb-2 block text-sm font-bold">Email atau nomor HP</label><input class="auth-input w-full rounded-2xl px-4 py-3.5 text-sm" type="text" id="login" name="login" value="{{ old('login') }}" required placeholder="nama@sekolah.sch.id atau 08..." autocomplete="username" @if($errors->has('email')) aria-invalid="true" aria-describedby="login-error" @endif>@error('email')<p id="login-error" class="mt-2 text-xs font-medium text-red-700" role="alert">{{ $message }}</p>@enderror</div>
